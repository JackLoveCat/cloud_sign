<template>
  <div class="app-container">
    ...
    <div class="operate-container" shadow="never" style="margin-top: 15px;">
      ...
      >
        <el-table-column align="center" label="ID" prop="dictTypeId">
        </el-table-column>
        <el-table-column align="center" label="字典类型" prop="dictType">
        </el-table-column>
        <el-table-column align="center" label="类型名称" prop="dictTypeName">
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="字典类型" prop="dictType">
          <el-input v-model="addForm.dictType" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item label="类型名称" prop="dictTypeName">
          <el-input
            v-model="addForm.dictTypeName"
            auto-complete="off"
          ></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    addSubmit () {
      ...
      this.$refs['addForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`sysdict/type`),
            method: 'post',
    ...
    handleDel (index, row) {
      ...
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl('sysdict/type/' + row.dictTypeId),
            method: 'delete',
            data: {}
          }).then(({ data }) => {
            if (data && data.code === 200) {
